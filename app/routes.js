...
module.exports = function(app, passport) {
   ...
   app.get('/netgame',isLoggedIn,function(req, res) {
        if ((req.cookies.name===undefined)&&(cookieflag===false)){           
            res.cookie("name", "player1");
            res.render('netgame.ejs',{
            user : req.user 
            });
            console.log(req.cookies.name);
            cookieflag=true;
        }else if ((req.cookies.name===undefined)&&(cookieflag===true)){
            res.cookie("name", "player2");
            res.render('netgame.ejs',{
            user : req.user 
            });
             console.log(req.cookies.name);
        }else if ((req.cookies.name==="player1")||(req.cookies.name==="player2")){
            res.render('netgame.ejs',{
            user : req.user 
            });
        }else{
